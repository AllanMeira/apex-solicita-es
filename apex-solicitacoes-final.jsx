...
function Avatar({ user, size=32 }) {
  ...
  if (user?.avatar) return <img src={user.avatar} alt={user.full_name} style={{ width:size, height:size, borderRadius:"50%", objectFit:"cover", flexShrink:0 }} />;
...
function AdminUsers({ bp, showToast }) {
  ...
  const saveUser=data=>{ const isEdit=users.some(u=>u.id===data.id); if(isEdit) setUsers(prev=>prev.map(u=>u.id===data.id?data:u)); else setUsers(prev=>[...prev,{...data,is_active:true}]); showToast(isEdit?"Usuário atualizado.":"Usuário criado!"); setModal(null); };
      ...
      <div style={{ display:"flex", height:"100vh", overflow:"hidden", fontSize:14, color:"#0f172a", background:"#f8fafc" }}>
        ...
        <div style={{ flex:1, overflow:"auto", display:"flex", flexDirection:"column", minWidth:0 }}>
          ...
          <main style={{ flex:1, padding:bp.isMobile?"16px 14px":"24px 24px", overflow:"auto" }}>
            ...
            {view==="admin-teams"&&(
              <div style={{ paddingBottom:bp.isMobile?80:0 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:18 }}><h2 style={{ fontSize:16, fontWeight:600, fontFamily:"'Outfit',sans-serif" }}>Equipes</h2><button style={{ padding:"8px 16px", background:"#1e3d6e", color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:600, fontFamily:"'DM Sans',sans-serif" }}>+ Adicionar</button></div>
                <div style={{ display:"grid", gridTemplateColumns:bp.isMobile?"1fr":"repeat(auto-fill,minmax(200px,1fr))", gap:12 }}>
                  {TEAMS.map(t=><div key={t.id} style={{ background:"#fff", borderRadius:12, border:"1px solid #e2e8f0", padding:"18px 20px", borderLeft:`4px solid ${t.color}` }}><div style={{ fontWeight:600, fontSize:15, marginBottom:4, fontFamily:"'Outfit',sans-serif" }}>{t.name}</div><div style={{ fontSize:12, color:"#64748b", marginBottom:10, fontFamily:"'DM Sans',sans-serif" }}>/{t.slug}</div><div style={{ display:"flex", alignItems:"center", gap:8 }}><div style={{ width:12, height:12, borderRadius:"50%", background:t.color }} /><span style={{ fontSize:11, color:"#94a3b8", fontFamily:"monospace" }}>{t.color}</span></div></div>)}
                </div>
              </div>
            )}
            {view==="admin-types"&&(
